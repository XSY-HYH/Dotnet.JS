...
// 自定义行编辑器，支持多行编辑、历史上下键、光标移动、实时语法高亮、Tab 补全
internal class LineEditor
{
    ...
    public LineEditor(Engine? engine = null, HistoryStore? history = null)
    {
        _engine = engine;
    ...
    private void Redraw()
    {
        var err = ErrorChecker.CheckSyntax(_line, 0);
        ...
        for (int li = 0; li < segments.Length; li++)
        {
            ...
            if (err != null && li == errLogicalRow)
                JsHighlighter.WriteErrorLine(segments[li], errColInRow);
            else
                JsHighlighter.WriteHighlighted(segments[li], _engine);
        }
    ...
    private void ShowCompletionList()
    {
        ...
        Console.WriteLine();
        foreach (var c in candidates) { JsHighlighter.WriteHighlighted(c, engine); Console.Write("  "); }
        Console.WriteLine();
        Redraw();
    }
    ...
    private List<string> CollectGlobalCandidates(Engine engine, string prefix)
    {
        ...
        foreach (var kw in JsHighlighter.Keywords)
